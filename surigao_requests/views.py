...
from .models import SurigaoRequestHeader, SurigaoRequestItems, AuthorizedPersons, Monitoring
from .forms import SurigaoRequestHeaderForm, SurigaoRequestItemsForm, AuthorizedPersonsForm, MonitoringForm
...
def dashboard(request):
    # monitoringData = SurigaoRequestItems.objects.all()
    if request.user.is_authenticated:
        monitoringData = Monitoring.objects.prefetch_related('header')
        
        return render(request, 'main/actions/dashboard.html', {'title':'Surigao', 'data':monitoringData})
    else:
...
def edit_authorizedPerson_view(request, person_id):
    if request.user.is_authenticated:
        ...
        return render(request, 'main/actions/edit_authorizedPerson.html', {'form':form, 'title':'Surigao'})
    else:
        return redirect('login')
...
def delete_person(request, person_id):
    if request.user.is_authenticated:
        ...
        # return redirect(reverse('Surigao_add_authorizedPerson', kwargs={'rs_number':person.header.rs_number}))
        return redirect(reverse(list_view_persons))
    else:
        return redirect('login')
...
def list_view_persons(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'main/actions/list_view_persons.html', {'persons':persons, 'title':'Surigao'})
    else:
        return redirect('login')
...
def delete_request(request, rs_number):
    if request.user.is_authenticated:    
        item = SurigaoRequestHeader.objects.get(pk=rs_number)
        item.delete()
        messages.success(request, "Deleted Successfully")
        return redirect(reverse(list_view_requests))
    else:
...
def list_view_report(request):
    if request.user.is_authenticated:    
        req = SurigaoRequestHeader.objects.all()
        return render(request, 'main/actions/print_reports.html', {'req':req, 'title':'Surigao'})
    else:
        return redirect('login')
