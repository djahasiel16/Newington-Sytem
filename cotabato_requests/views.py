...
from .models import CotabatoRequestHeader, CotabatoRequestItems, AuthorizedPersons, Monitoring
from .forms import CotabatoRequestHeaderForm, CotabatoRequestItemsForm, AuthorizedPersonsForm, MonitoringForm
...
def dashboard(request):
    # monitoringData = CotabatoRequestItems.objects.all()
    if request.user.is_authenticated:
        monitoringData = Monitoring.objects.prefetch_related('header')
        
        return render(request, 'main/actions/dashboard.html', {'title':'Cotabato', 'data':monitoringData})
    else:
...
def edit_authorizedPerson_view(request, person_id):
    if request.user.is_authenticated:
        ...
        return render(request, 'main/actions/edit_authorizedPerson.html', {'form':form, 'title':'Cotabato'})
    else:
        return redirect('login')
...
def list_view_persons(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'main/actions/list_view_persons.html', {'persons':persons, 'title':'Cotabato'})
    else:
        return redirect('login')
...
def list_viewRequest_items(request, rs_number):
    ...
    items = CotabatoRequestItems.objects.filter(header=CotabatoRequestHeader.objects.get(pk=rs_number))
    rs = CotabatoRequestHeader.objects.get(pk=rs_number)
    return render(request, 'main/actions/list_viewRequest_items.html', {'data':items, 'rs_header':rs,'title':'Cotabato', 'disabled':disabled})
...
def delete_request(request, rs_number):
    if request.user.is_authenticated:    
        item = CotabatoRequestHeader.objects.get(pk=rs_number)
        item.delete()
        messages.success(request, "Deleted Successfully")
        return redirect(reverse(list_view_requests))
    else:
...
def list_view_report(request):
    if request.user.is_authenticated:    
        req = CotabatoRequestHeader.objects.all()
        return render(request, 'main/actions/print_reports.html', {'req':req, 'title':'Cotabato'})
    else:
        return redirect('login')
